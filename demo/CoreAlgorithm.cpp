...
CoreAlgorithm::CoreAlgorithm(const std::string &path, CameraArguments *cArgs) {
	...
	// 转换为hsv空间， H代表表示光谱上的颜色，
	// S表示饱和度，向颜色中添加白色，添加的越多越不饱和；v表示明度，表示往颜色里加黑色，明度越低越暗。
	hsv = image.clone();
	...
	for (auto i = 0; i < maximas.size(); i++) {
		...
		for (auto j = 0; j < maximas[i].size(); j++) {
			...
			if (tang.at<float>(2, 0) > 750 && tang.at<float>(2, 0) < 1500) {
				...
				int r = (int)rgbChannel[2].at<uchar>(i + minX, maximas[i][j]), g = rgbChannel[1].at<uchar>(i + minX, maximas[i][j]),
			...
			for (auto k = mark; k < phases[i].size() - 1; k++) {
				...
				if (tang.at<float>(2, 0) > 750 && tang.at<float>(2, 0) < 1500) {
					coordinate.push_back(tang.t());
					int r = (int)rgbChannel[2].at<uchar>(i + minX, (start + k)), g = rgbChannel[1].at<uchar>(i + minX, (start + k)),
				...
				if ((start + k) > maximas[i][j] && phases[i][k] < 0 && phases[i][k + 1] > 0 && pi)
					break;
			}
		}
	}
...
void CoreAlgorithm::run() {
	...
	for (auto i = 0; i < rows; i++) {
		for (auto j = 0; j < cols; j++) {
			mask.at<float>(i, j) = (int)rgbChannel[0].at<uchar>(i, j) > (int)rgbChannel[1].at<uchar>(i, j) ?
			                           ((int)rgbChannel[0].at<uchar>(i, j) > (int)rgbChannel[2].at<uchar>(i, j) ? (int)rgbChannel[0].at<uchar>(i, j) :
			                                                                                                      (int)rgbChannel[2].at<uchar>(i, j)) :
			                           ((int)rgbChannel[1].at<uchar>(i, j) > (int)rgbChannel[2].at<uchar>(i, j) ? (int)rgbChannel[1].at<uchar>(i, j) :
			                                                                                                      (int)rgbChannel[2].at<uchar>(i, j));
		}
	}
	...
	for (auto i = minX; i < maxX; i++) {
		for (auto j = minY; j < maxY; j++) {
			img.at<float>(i, j) = 0.2989 * (int)rgbChannel.at(2).at<uchar>(i, j) + 0.5907 * (int)rgbChannel.at(1).at<uchar>(i, j) +
	...
	// color label
	// 反应了在x的一定范围内，即一定宽度的颜色是什么，给这一列打赏了标签
	...
	for (auto i = minX; i < maxX; i++) {
		...
		for (auto j = 0; j < x_size[1]; j++) {
			...
			// 然后，遍历数组中的每一个元素，将其除以π再乘以7，将其范围变为[-7,
			// 7]，并将结果存储在phases向量的当前行中。
