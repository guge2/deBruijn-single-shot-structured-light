...
void Reconstruction(vector<vector<float>> maximas,
                    ...
                    /*vector<vector<float>> phases, */ const Mat &Hc1, Mat Hp2,
                    const double *map) {
...
void savePly() {
  ...
  for (const auto &point : coordinate) {
    plyFile << point.at<float>(0, 0) << " " << point.at<float>(0, 1) << " "
            << point.at<float>(0, 2) << std::endl;
  }
...
int main() {
  // 旋转矩阵
  Mat r(3, 3, CV_32F);
  double m0[3][3] = {{9.7004457782050868e-001, 1.3447278830863673e-002,
                      2.4255450466457243e-001},
                     {-8.7082927494022376e-003, 9.9974988338843274e-001,
                      -2.0599424802792338e-002},
                     {-2.4277084396282392e-001, 1.7870124701864658e-002,
                      9.6991905639837694e-001}};
  for (auto i = 0; i < r.rows; i++)
    for (auto j = 0; j < r.cols; j++)
      r.at<float>(i, j) = m0[i][j];
  // 平移矩阵
  Mat t(1, 3, CV_32F);
  double m1[1][3] = {{-1.9511179496234658e+002, 1.2627509817628756e+001,
                      -5.9345885017522171e+001}};
  ...
  // 左相机内参
  Mat kc(3, 3, CV_32F);
  double m2[3][3] = {{2.1536653255083029e+003, 0., 6.1886776197116581e+002},
  ...
  // 右相机内参
  Mat kp(3, 3, CV_32F);
  double m3[3][3] = {{1.7235093158297350e+003, 0., 4.4128195628736904e+002},
  ...
  rgbChannel =
      imread("..\\Data\\image\\reconstruction\\test.png", cv::IMREAD_UNCHANGED);
  ...
  for (auto i = 0; i < rows; i++) {
    for (auto j = 0; j < cols; j++) {
      mask.at<float>(i, j) = (int)rgbChannel.at<Vec3b>(i, j)[0] >
